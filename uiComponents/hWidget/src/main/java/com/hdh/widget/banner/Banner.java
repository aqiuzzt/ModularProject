...
import com.hdh.widget.banner.listener.OnBannerClickListener;
import com.hdh.widget.banner.listener.OnBannerListener;
import com.hdh.widget.banner.loader.ImageLoaderInterface;
import com.hdh.widget.banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    public Banner setCustomAdapter(PagerAdapter adapter) {
    ...
    private void setTitleStyleUI() {
        if (!isCustomAdapter && titles.size() != imageUrls.size()) {
    ...
    private void setData() {
        ...
        if (!isCustomAdapter && adapter == null) {
...
//                Log.i(SECRET, "curr:" + currentItem + " count:" + count);
                if (currentItem == 1) {
...
//        Log.i(SECRET, ev.getAction() + "--" + isAutoPlay);
        if (isAutoPlay) {
    ...
    public void onPageSelected(int position) {
        ...
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
                ...
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE) {
            if (isCustomAdapter) {
                indicatorImages.get(lastPosition % count).setImageResource(mIndicatorUnselectedResId);
