...
public class GridDividerDecoration extends RecyclerView.ItemDecoration {
    ...
    public GridDividerDecoration(Context context, @Orientation int orientation, int headerCount) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        final int childCount = parent.getAdapter().getItemCount() - mHeaderCount;
    ...
    private boolean isLastColumn(int position, int spanCount, int childCount) {
        if (mOrientation == GRID_DIVIDER_VERTICAL) {
            return (position + 1 - mHeaderCount) % spanCount == 0;
        } else {
    ...
    private boolean isLastRow(int position, int spanCount, int childCount) {
        if (mOrientation == GRID_DIVIDER_VERTICAL) {
            childCount = childCount - mHeaderCount;
            int lastColumnCount = childCount % spanCount;
            lastColumnCount = lastColumnCount == 0 ? spanCount : lastColumnCount;
            return (position - mHeaderCount) >= childCount - lastColumnCount;
        } else {
    ...
    private int getSpanCount(RecyclerView parent) {
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ((GridLayoutManager) layoutManager).getSpanSizeLookup().getSpanSize(0);
            return ((GridLayoutManager) layoutManager).getSpanCount();
        } else if (layoutManager instanceof StaggeredGridLayoutManager) {
            return ((StaggeredGridLayoutManager) layoutManager).getSpanCount();
        } else {
            throw new UnsupportedOperationException("the GridDividerDecoration can only be used in " +
    ...
    public void drawVerticalDividers(Canvas c, RecyclerView parent) {
        ...
        final int childCount = parent.getChildCount() - mHeaderCount;
        for (int i = mHeaderCount; i < childCount; i++) {
